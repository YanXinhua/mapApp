...
;(function (global, factory) {
  typeof exports === 'object' && typeof module !== 'undefined'
    ? (module.exports = factory(require('myopenlayers')))
    : typeof define === 'function' && define.amd
      ? define(['myopenlayers'], factory)
...
*/
  ...
  HDMap.prototype.addMarker = function (markerInfo, styleObj) {
    ...
    iconFeature.setStyle(
      hdmap.commonConfig.getFeatureStyle(markerInfo, styleObj)
    )
  ...
  HDMap.prototype.addBgLayer = function (markerInfo) {
    ...
    if (markerInfo.markerType === 'countWarning') {
      iconBgFeature.setStyle(
        hdmap.commonConfig.getCountWarningStyle(markerInfo)
      )
    } else if (markerInfo.markerType === 'countCamera') {
      iconBgFeature.setStyle(
        hdmap.commonConfig.getCountDefaultStyle(markerInfo)
      )
    } else if (markerInfo.markerType === 'countBroadcast') {
      iconBgFeature.setStyle(
        hdmap.commonConfig.getCountDefaultStyle(markerInfo)
...
*/
  ...
  HDMap.prototype.showPopup = function (domId, coordinate) {
    if (domId) {
      var wrapY = parseInt(
        document.getElementById(domId).parentNode.style.height
      )
  ...
  HDMap.prototype.popupMultipoint = function (coordinate, features) {
    ...
    for (var i = 0; i < features.length; i++) {
      ...
      contentImg.src = features[i].extProperties.markerImg
      ...
      contentDetail.innerHTML = features[i].extProperties.markerName
    ...
    var _loop = function _loop (_i) {
      ...
      _this.popup_contentWrap[_i].onclick = function () {
        ...
        // });
        hdmap.showPopup(features[_i].extProperties.markerType, coordinate)
        // hdmap.popupSinglepoint(
        //   coordinate,
        //   features[i],
        //   features[i].extProperties.markerType
        // );
      }
    }
  ...
  HDMap.prototype.warnAnimation = function (areaInfo) {
    ...
    if (this._flag) {
      this.getMap().on('precompose', function () {
        ...
        that.updateArea(
          areaInfo,
          hdmap.commonConfig.getWarningAreaStyle(opacity)
        )
        that._flag = false
      })
   ...
   */
  HDMap.prototype.regEventListener = function (
    eventType,
    callback,
    featureType
  ) {
...
*/
  ...
  HDMap.prototype.showDrawLine = function (option) {
    ...
    if (layer) {
      ...
      for (var i = 0, len = features.length; i < len; i++) {
        areaFeaturesArr = areaFeaturesArr.concat(
  ...
  HDMap.prototype.initDrawShapeTool = function (type) {
    ...
    if (!this.drawTool.shapeDraw) {
      ...
      this.drawTool.shapeModify = new ol.interaction.Modify({ source: source })
  ...
  HDMap.prototype.showDrawShape = function (option) {
    ...
    if (layer) {
      ...
      var aa = []
      for (var i = 0, len = features.length; i < len; i++) {
        aa = aa.concat(features[i].getGeometry().getCoordinates()[0])
      }
      areaFeaturesArr.push(aa)
      var _id = option && option.id ? option.id : ''
      var _name = option && option.name ? option.name : ''
      var _areaType = option && option.areaType ? option.areaType : ''
      if (!_id) console.warn('showDrawShape Error : shape id can not be empty')
      areaFeature = this.addArea({
        id: _id,
        name: _name,
        areaType: _areaType,
        borderPoints: areaFeaturesArr
...
*/
  ...
  HDMap.prototype.transfromWGSToBitMap = function (position) {
    ...
    if (this.mapConfig.scaleType !== 1) {
      console.warn('this map cannot translate GPS to XYZ')
      return
    }
  ...
  function getDistanceByPoint (map, lonlats, points) {
    ...
    if (
      ...
      lonlats['lonlatA'][0] < -180 ||
      lonlats['lonlatA'][1] > 90 ||
      lonlats['lonlatA'][1] < -90 ||
      lonlats['lonlatB'][0] > 180 ||
      lonlats['lonlatB'][0] < -180 ||
      lonlats['lonlatB'][1] > 90 ||
      lonlats['lonlatB'][1] < -90 ||
      lonlats['lonlatC'][0] > 180 ||
      lonlats['lonlatC'][0] < -180 ||
      lonlats['lonlatC'][1] > 90 ||
      lonlats['lonlatC'][1] < -90
    ) {
      console.warn(
        'Error: Longitude range: -180 to 180, latitude range: -90 to 90'
  ...
  function judgePointInsidePolygon (point, poly) {
    function rayMethod (point, poly) {
      for (
        var f = false, i = 0, l = poly.length, j = l - 1;
        i < l;
        j = i, i++
      ) {
        // 点与多边形顶点重合
